...
def main():
    ...
    sample_batches = [
        samples[i * batch_size : min((i + 1) * batch_size, len(samples))]
        for i in range(NUM_PROCESSES)
    ]
    material_batches = [
        material_samples[i * batch_size : min((i + 1) * batch_size, len(material_samples))]
        for i in range(NUM_PROCESSES)
    ]
    index_batches = [
        list(range(i * batch_size, min((i + 1) * batch_size, len(samples))))
        for i in range(NUM_PROCESSES)
    ]

    # Prepare argument tuples for each process
    args = [
        (reference_pulse, material_batches[i], index_batches[i], deltat)
        for i in range(NUM_PROCESSES)
    ]
    ...
    }, "regression_data/train_3_layer_100k_nonoise_n1to8.pt")

    print("Dataset saved successfully as train_3_layer_100k_nonoise_n1to8.pt in regression_data")
